...
| **Setup-token (OAuth)** | Uses Claude Max/Pro subscription quota | Under investigation (see below) | Subscription users, fixed monthly cost |
...
> **Prompt caching status with setup-token:** Research suggests both auth methods use the same backend caching infrastructure — OAuth tokens generated by `setup-token` route through the same API as direct API keys. However, subscription billing is quota-based (not per-token), so caching savings are invisible in billing but should still reduce quota consumption per message. **This needs verification via ClawMetry** — monitor `cache_read_input_tokens` after switching to confirm caching is active. If cache reads show 0, fall back to API key auth for production use.
>
